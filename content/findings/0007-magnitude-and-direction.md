...
GitLab has the concept of [Internal users](https://docs.gitlab.com/administration/internal_users/) which we can think of as "bots". These are designed to be used for automated and background transactions.
...
The problem stems from the fact that any Project Access Token can [have access to all other projects](https://gitlab.com/gitlab-org/gitlab/-/work_items/413028#note_1499970536) which we highlighted in our documentation. We do have an open [issue](https://gitlab.com/gitlab-org/gitlab/-/work_items/383882) so that this can be addresssed in future iterations of the product. That said, it's fair to say that their original implementation was bound to fail given the limitation.
...
- In the interim, we also floated the idea of only allowing administrators to create Project and Group Access tokens using the [Rails console](https://docs.gitlab.com/administration/operations/rails_console/).
...
- We helped them move their internal projects to private visibility.
